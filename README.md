...
Superseded nodes fold under the rule that covers them: click `covers N` on a rule to open
it, or switch to `all` to see everything at once. The strip at the top gives you the same
`N rules over M specifics`, and the same budget, that `knoten frontier` opens with.
`compressible` outlines the clusters that could become one rule.
