...
def _order(nodes: dict) -> list:
    ...
    always sorts last, so it can only ever be added to the end of a column or the rim of
    a cluster."""
    return sorted(nodes.values(),
                  key=lambda n: (str(n.frontmatter.get("created") or ""), n.id))
...
def roles(nodes: dict) -> tuple:
    ...
    is a shelf, and belongs at the start.
    """
...
def _map(nodes: dict) -> tuple:
    ...
    Honest limit: degree changes as a graph grows, so a node that gains edges can be
    promoted to hub and shift the ring. Appending a leaf — the common case, and what an
    agent loop does — moves nothing.
    """
    if not nodes:
        return {}, {}
...
def payload(root: Path) -> dict:
    ...
    return {
        ...
        "count": len(nodes),
        "columns": cols,
        "gate_types": sorted(gates & set(cols)),
        ...
        "nodes": [{
            ...
            "sections": [{"title": t, "text": (section(n.body, t) or "")[:1500]}
                         for t in n.sections],
...
def render(root: Path) -> str:
    ...
    return (HERE / "viz.html").read_text(encoding="utf-8") \
        .replace("__KNOTEN_DATA__", blob).replace("__ROOT__", root.name)
