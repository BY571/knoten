...
def test_a_rejected_commit_exits_nonzero(graph, tmp_path, monkeypatch, capsys):
    graph.rules("name: t\nstatuses: [open]\nnode_types: [hypothesis]\nrules: []\n")
    monkeypatch.chdir(graph.root)
...
def test_a_rejected_commit_reason_is_on_stderr(graph, tmp_path, monkeypatch, capsys):
    graph.rules("name: t\nstatuses: [open]\nnode_types: [hypothesis]\nrules: []\n")
    monkeypatch.chdir(graph.root)
    (tmp_path / "fm").write_text("type: hypothesis\nstatus: bogus", encoding="utf-8")
    (tmp_path / "body").write_text("# x\n", encoding="utf-8")

    main(["commit", "hyp-x", "--frontmatter", str(tmp_path / "fm"),
          "--body", str(tmp_path / "body")])
...
def test_a_rejected_commit_json_payload_is_on_stdout(graph, tmp_path, monkeypatch, capsys):
    graph.rules("name: t\nstatuses: [open]\nnode_types: [hypothesis]\nrules: []\n")
    monkeypatch.chdir(graph.root)
    (tmp_path / "fm").write_text("type: hypothesis\nstatus: bogus", encoding="utf-8")
    (tmp_path / "body").write_text("# x\n", encoding="utf-8")
...
def test_update_moves_the_status(graph, monkeypatch):
    graph.rules(RULES).node("hyp-x", "id: hyp-x\ntype: hypothesis\nstatus: open", "# c\n")
    monkeypatch.chdir(graph.root)

    assert main(["update", "hyp-x", "--status", "dead"]) == 0
    assert load(graph.root)["hyp-x"].status == "dead"
...
def test_update_appends_from_a_file(graph, tmp_path, monkeypatch):
    graph.rules(RULES).node("hyp-x", "id: hyp-x\ntype: hypothesis\nstatus: open", "# c\n")
    monkeypatch.chdir(graph.root)
    ...
    assert code == 0
    assert "Why it died" in (graph.root / "nodes" / "hyp-x.md").read_text(encoding="utf-8")


def test_update_reports_a_refusal_without_a_traceback(graph, monkeypatch, capsys):
    graph.rules(RULES).node("hyp-x", "id: hyp-x\ntype: hypothesis\nstatus: open", "# c\n")
    monkeypatch.chdir(graph.root)
...
def test_a_refused_update_json_payload_is_on_stdout(graph, monkeypatch, capsys):
    graph.rules(RULES).node("hyp-x", "id: hyp-x\ntype: hypothesis\nstatus: open", "# c\n")
    monkeypatch.chdir(graph.root)
...
def test_a_malformed_kv_flag_is_a_graph_error_not_a_crash(graph, monkeypatch, flag, bad):
    graph.rules(RULES).node("hyp-x", "id: hyp-x\ntype: hypothesis\nstatus: open", "# c\n")
    monkeypatch.chdir(graph.root)
