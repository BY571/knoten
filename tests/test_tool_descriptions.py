...
async def test_every_tool_has_a_description():
    assert all(t.description for t in await mcp_server.list_tools())
...
async def test_every_tool_appears_in_the_loop_instructions():
    ...
    missing = [t.name for t in await mcp_server.list_tools() if t.name not in text]
...
async def test_only_one_tool_claims_to_be_the_starting_point():
    ...
    shouty = [t.name for t in await mcp_server.list_tools()
...
async def test_query_does_not_promise_more_than_keyword_search_can_do():
    ...
    desc = next(t.description for t in await mcp_server.list_tools()
